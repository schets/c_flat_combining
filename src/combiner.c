...
enum { Waiting = 0, TakeOver = 1, Finished = 2 };
...
// Combiner functions
void do_unlock_combiner(struct combiner *cmb, struct message_metadata *head,
                        int do_work);
int __attribute__((noinline))
enter_combiner(struct combiner *cmb, struct message_metadata *msg);
void __attribute__((noinline))
notify_waiters(struct combiner *cmb, struct message_metadata *stop_at);
void do_release(struct combiner *cmb);
struct message_metadata *perform_work(struct combiner *cmb,
...
static void prefetch(void *p) { __builtin_prefetch(p, 0, 3); }
...
int enter_combiner(struct combiner *cmb, struct message_metadata *msg) {
  ...
  struct message_metadata *prev =
      __atomic_exchange_n(&cmb->queue, msg, __ATOMIC_ACQ_REL);
...
struct message_metadata *perform_work(struct combiner *cmb,
  ...
  do {
    ...
    if (nrun < MAX_RUN || cur_msg->operation != NULL) {
      ...
      // as of now,
      // the first item will always be the current thread's node from list entry
      // or
      // take over
...
struct message_metadata *next_list(struct message_metadata **queue,
                                   ...
                                   int advance_end) {
  struct message_metadata *next =
      __atomic_load_n(&head->next, __ATOMIC_CONSUME);
  if (next == NULL) {
    __atomic_thread_fence(__ATOMIC_ACQUIRE);
    struct message_metadata *tmp_head = head;
    if (!advance_end ||
        __atomic_compare_exchange_n(queue, &tmp_head, NULL, 1, __ATOMIC_RELEASE,
                                    __ATOMIC_RELAXED)) {
      next = NULL;
    } else {
